...
long lastIncomingMillis = 0;
bool isWin10Connected = false;
...
void readSerial() {
	...
	if (Serial.available()>0) {
		lastIncomingMillis = millis();
		isWin10Connected = true;
		isAvailable = true;
...
void serialHandle() {
	...
	if (isWin10Connected) {
		if (millis() - 3000 > lastIncomingMillis) {
			isWin10Connected = false;
			rgb.clear();
			rgb.setColor(10, 0, 0);
		}
	}
